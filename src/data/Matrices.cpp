...
///////////////////////////////////////////////////////////////////////////////
// Modified July 7, 2018 by Derelict Vessel Dev for "Fourth Realm".
// - Added Matrix4 constructor to build view matrix.
///////////////////////////////////////////////////////////////////////////////
...
///////////////////////////////////////////////////////////////////////////////
// Construct a rotation matrix with given angle (degree) and rotation axis
///////////////////////////////////////////////////////////////////////////////
Matrix3::Matrix3(float angle, const Vector3& axis)
{
    float c;
    float s;
    float c1;

    // Avoid sin/cos calculation for simple cases.
    if (fabs(angle) <= EPSILON)
    {
        c = 1.0f; s = 0.0f;
    }
    else if (fabs(angle - 90.0f) <= EPSILON)
    {
        c = 0.0f; s = 1.0f;
    }
    else if (fabs(angle - 180.0f) <= EPSILON)
    {
        c = -1.0f; s = 0.0f;
    }
    else if (fabs(angle - 270.0f) <= EPSILON)
    {
        c = 0.0f; s = -1.0f;
    }
    else
    ...
    m[3] = axis.x * axis.y * c1 - axis.z * s;
    m[4] = axis.y * axis.y * c1 + c;
    m[5] = axis.y * axis.z * c1 + axis.x * s;
    m[6] = axis.x * axis.z * c1 + axis.y * s;
    m[7] = axis.y * axis.z * c1 - axis.x * s;
    m[8] = axis.z * axis.z * c1 + c;
}
...
///////////////////////////////////////////////////////////////////////////////
Matrix4::Matrix4(float yFovInDegrees, float aspect, float near, float far)
{
    const float a = ( 1.0f / tanf( (yFovInDegrees * DEG2RAD) / 2.0f ) );
