...
struct Vector3
{
    ...
    Vector3&    normalize();                            //
    Vector3&    limit(const Vector3& max);
...
struct Vector4
{
    ...
    void        set(const Vector3& vec, float w);
    float       length() const;                         //
...
inline Vector3& Vector3::limit(const Vector3& max)
{
    if (abs(x) > max.x)
        x = max.x * (x >= 0.0f ? 1.0f : -1.0f);
    if (abs(y) > max.y)
        y = max.y * (y >= 0.0f ? 1.0f : -1.0f);
    if (abs(z) > max.z)
        z = max.z * (z >= 0.0f ? 1.0f : -1.0f);
...
inline void Vector4::set(const Vector3& vec, float w) {
    this->x = vec.x; this->y = vec.y; this->z = vec.z; this->w = w;
}
